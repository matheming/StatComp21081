...
#antithetic variable
set.seed(2001)
Result_ant<-Gen_Ray_ant(sigma = 2,n=1000,antithetic = TRUE)
X1_ant<-Result_ant$X1
X2_ant<-Result_ant$X2
print(var_ant<-var((X1_ant+X2_ant)/2))
#non-antithetic
set.seed(2002)
Result_non<-Gen_Ray_ant(sigma = 2,n=1000,antithetic = FALSE)
...
```
The CP of the symmetric $t$-interval is `r mean(U<2&V>2)` and the CP of UCL is `r mean(UCL>2)`, which is smaller than 0.95 and smaller than the result in Example 6.4. Therefore, when the distribution is non-normal, the CP of the $t$-interval is smaller than that of the interval for variance, the $t$-interval is more robust.
...
print(bias.Boot)
```
The estimation of $\theta$ is `r Theta`, with 10000 bootstrap, the estimation of the bias is `r bias.Boot` and the standard error is `r se.R`.
...
# empirical CP
CPnm.norm<-CPnm.perc<-CPnm.basic<-numeric(1e3)
for (i in 1:1e3) {
# normal population
nmdata<-rnorm(size)
boot.nm<-boot(nmdata,statistic = sk,R=1e3)
...
# chi distribution
# empirical CP
CPchi.norm<-CPchi.perc<-CPchi.basic<-numeric(1e3)
for (i in 1:1e3) {
  datachi<-rchisq(size,5)
  # chi population
  bootchi<-boot(datachi,statistic = sk,R=1e3)
